...
class Wp_Book {
	...
	private function load_dependencies() {
		...
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/widget.php';
	...
	private function define_admin_hooks() {
		...
		$this->loader->add_action( 'init', $plugin_admin, 'create_wpbook_cpt' );
		$this->loader->add_action( 'init', $plugin_admin, 'create_bookcategory_tax' );
		$this->loader->add_action( 'init', $plugin_admin, 'create_booktag_tax' );

		$this->loader->add_action( 'add_meta_boxes', $plugin_admin, 'create_book_meta_boxes' );
		$this->loader->add_action( 'save_post_wpbook',$plugin_admin, 'wp_book_meta_box_save');
		$this->loader->add_action('init',$plugin_admin, 'wp_register_metadata_table', 0);
		$this->loader->add_action( 'switch_blog', $plugin_admin,'wp_register_metadata_table',0 );

		$this->loader->add_action( 'admin_menu', $plugin_admin, 'wp_book_setting_menu' );
		$this->loader->add_action( 'admin_init', $plugin_admin, 'wp_book_setting_register' );
		
		add_shortcode( 'wpbook', array( $plugin_admin, 'wp_book_shortcode' ) );


		$this->loader->add_action('widgets_init',$plugin_admin,'wp_book_register_widget');
		$this->loader->add_action('wp_dashboard_setup',$plugin_admin,'wp_book_dashboard_widget');
	}
